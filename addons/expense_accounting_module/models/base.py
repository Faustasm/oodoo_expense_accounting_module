...
class BaseModel(models.Model):
    ...
    converted_amount = fields.Monetary(
        string="Amount (USD)", currency_field="converted_amount_currency_id"
    )
    converted_amount_currency_id = fields.Many2one(
        "res.currency", string="Currency", domain=[("name", "in", ("USD"))]
    )
    ...
    def _update_converted_amount(self):
        ...
        self.converted_amount = converter.convert(
            self.amount, self.currency_id.name, "USD"
        )
